...
/* uchar2float.c
 * Converts a raw 8-bit unsigned Char Image
 * to a raw IEEE Floating point Image .
 * Image values between 0-255 expected, with
 * output values between 0.0-2.0 only.
 * Grayscale only. */
...
int read_raw(const char *fname, IMAGE *data)
{
	...
	data->imguc = malloc((data->xmax)*(data->ymax)*(data->zmax)*sizeof(UCPIXEL));	// allocate memory
	if(!data->imguc) return -1;		// exit if operation failed

	F = fopen(fname, "rb");			// open file for read

	data->elems = fread(data->imguc, sizeof(UCPIXEL), (data->xmax)*(data->ymax)*(data->zmax), F);
...
int write_raw(const char *fname, IMAGE *data)
{
	...
	data->elems = fwrite(data->imgf, sizeof(FPIXEL), (data->xmax)*(data->ymax)*(data->zmax), F);
	if(data->elems!=(data->xmax)*(data->ymax)*(data->zmax))
		return -1;			// size mismatch error
...
/* ***************************************************
 * Function   : uchar2float
 * Arguments  : IMAGE *target, IMAGE *data
 * Returns    : 0 for success
 * Description: Converts 8-bit uchar to Grayscale 
 * 		IEEE Grayscale Float (img vals 0-2)
 * **************************************************/
int uchar2float(IMAGE *target, IMAGE *data)
{
  int i;
  float maxvalin = 255.0;	 // maxval of input image elements
  float maxvalout =  2.0;	 // maxval of output image elements
  float scaler = maxvalout/maxvalin;  // conversion factor 
  printf("   uchar2float conversion...\n");

  for(i=0; i<data->elems; i++)
  {
	  target->imgf[i] = (float)(data->imguc[i]) * scaler;
  }
